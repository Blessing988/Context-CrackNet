...
Also provides a SegmentationMetrics class for accumulating metrics over batches.
"""
...
def calculate_iou(pred_mask, true_mask, num_classes):
    ...
    if num_classes == 1:
        ...
        if union == 0:
            ious.append(float('nan'))  # Ignore if no ground truth
        else:
...
def calculate_dice(pred_mask, true_mask, num_classes):
    ...
    Dice = 2 * intersection / (sum_pred + sum_true)
...
def calculate_precision_recall(pred_mask, true_mask, num_classes):
    ...
    Returns:
        tuple: (precisions, recalls) lists for each class
    """
    precisions = []
    recalls = []
    pred_mask = pred_mask.view(-1)
...
def evaluate_metrics(pred_mask, true_mask, num_classes):
    """
    Evaluate all segmentation metrics.
    ...
    Returns:
        dict: Dictionary with IoU, Dice, Precision, Recall, F1 Score
    """
...
def save_metrics(epoch, train_loss, train_metrics, val_loss, val_metrics, output_csv):
    ...
    Args:
        ...
        train_metrics: List of training metrics [IoU, Dice, Precision, Recall, F1]
...
class SegmentationMetrics:
    """
    Class for accumulating segmentation metrics over multiple batches.
    
    Useful for computing epoch-level metrics without storing all predictions.
    ...
    def compute_metrics(self):
        ...
        Returns:
            dict: Dictionary with per-class and weighted metrics
        """
        metrics = {}
...
def evaluate_batch(pred_mask, true_mask, num_classes):
    ...
    Returns:
        dict: Computed metrics for the batch
    """
